  ...
  if (window.location.href.indexOf("catalogue/moredetail.pl") > -1) {
    ...
    Promise.all([promise1]).then(() => {
      const itemElements = document.querySelectorAll(
        "#catalogue_detail_biblio h3[id^=item]",
      );
...
class ItemMessages extends React.Component {
  ...
  removeMessage = (index) => {
    ...
    $.ajax({
      url:
        "/api/v1/contrib/item_messages/items/" +
        message.itemnumber +
        "/messages/" +
        message.item_message_id,
      type: "DELETE",
      success: function () {
    ...
    return html`<div className="listgroup">
      ...
      <ol className="bibliodetails">
        ${this.state.messages.map(function (message, index) {
          return html`<${ItemMessage}
            key=${message.item_message_id}
            message=${message}
            index=${index}
            onRemove=${item_messages.removeMessage}
          />`;
        })}
        <${ItemMessageCreator}
          itemnumber=${this.props.itemnumber}
          onAdd=${item_messages.addMessage}
        />
      </ol>
    </div>`;
  };
...
class ItemMessage extends React.Component {
  ...
  render = () => {
    return html`<li>
      ...
      <button href="#" onClick=${this.removeMessage}>
...
class ItemMessageCreator extends React.Component {
  ...
  render = () => {
    ...
    if (lib_opac) {
      ...
      >
        ${av_options.map(
          (av) => html`<option key=${av} value=${av}>${av}</option>`,
        )}
      </select>`;
    } else {
      pulldown_or_text = html`<input
        style=${{ width: "15em", margin: ".5em" }}
        className="input-xlarge"
        type="text"
        value=${this.state.message}
